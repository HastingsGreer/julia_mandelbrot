...
# ╔═╡ 599c35d6-7359-11ec-3d58-f9adb7a72490
begin
	using JLD
...
# ╔═╡ 69cfa3c8-fb57-4e29-938c-37c594f5b9a8
md"""
maxiter
$(@bind maxiter Slider(1:60000))
"""

# ╔═╡ f29513b4-bb8c-4850-9929-b5e751504411
...
# ╔═╡ 5b033c2c-6805-4f0e-a01f-0259f8c915b2
begin
...
# ╔═╡ ba441497-9d5f-4211-8c58-be773d7b06d4
begin
...
# ╔═╡ 3157cfda-2ea1-4537-9485-41359dfcd982
begin
	...
	render(out)#, inititers, minimum(out), maximum(out)
end

# ╔═╡ 01c0d4fe-417f-4a88-8037-a0035342c94b
begin
	coefficients = Array{Complex{Float64}, 1}([0, 0, 0, 0])
	new_coefficients, iters = series_iterate(coefficients, Complex{Float64}(center), radius, maxiter)
	delta_arr = range(-radius, radius, length=512)
	delta_arr = delta_arr .+ im .* delta_arr'

	z = map(delta_arr) do delta
		z_start = evaluatePolynomial(new_coefficients, delta)
		inner_loop(iters + 5000, iters, z_start, Complex{Float64}(center) + delta)
	end
...
# ╔═╡ 23c33e00-c922-43d2-b498-7a4df2d10554
iters, minimum(z_mag), maximum(z_mag)
